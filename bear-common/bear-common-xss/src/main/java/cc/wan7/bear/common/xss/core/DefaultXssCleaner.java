...
package cc.wan7.bear.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import cc.wan7.bear.common.xss.config.BearXssProperties;
import cc.wan7.bear.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final BearXssProperties properties;

	public DefaultXssCleaner(BearXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(BearXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		BearXssProperties.Mode mode = properties.getMode();
		if (BearXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (BearXssProperties.Mode.validate == mode) {
			// 校验
